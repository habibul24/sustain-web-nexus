...
-- Update the original set_emission_factors function to use the correct Towngas emission factor
...
BEGIN
  ...
  -- Set emission factors for Towngas - Updated to use 0.548
  IF NEW.source_of_energy = 'Towngas' THEN
    NEW.carbon_dioxide_emitted_co2 = 0.548;
    NEW.gwp_co2e = 1;
    ...
    -- Set prior year emission factor if column exists
    IF TG_TABLE_SCHEMA = 'public' AND TG_TABLE_NAME = 'stationary_combustion' THEN
...
-- Update existing Towngas records to use the correct emission factor
UPDATE stationary_combustion
SET 
  emission_factor = 0.548,
  carbon_dioxide_emitted_co2 = 0.548,
  emission_factor_prior_year = 0.549
WHERE source_of_energy = 'Towngas';
